...
async function syncQueue() {
  ...
  try {
    ...
    // Filter: not yet sent (in-memory) AND not already marked in Firebase
    const toSend = (qSnap.data().tracks || []).filter(t =>
      !queuedTrackIds.includes(t.id) && !t.sentToSpotify
    );

    for (const track of toSend) {
      if (!track.uri) continue;

      // Mark as sent in Firebase BEFORE sending to Spotify.
      // This prevents a second server instance (during Render deploy overlap)
      // from also sending the same song.
      try {
        const qRef = db.collection('state').doc('queue');
        const marked = await db.runTransaction(async tx => {
          ...
          if (tracks[idx].sentToSpotify) return false; // already sent by another instance
          tracks[idx] = { ...tracks[idx], sentToSpotify: true };
          tx.set(qRef, { tracks, updated: Date.now() });
          return true;
        });
        if (!marked) {
          console.log(`⏭️  Already sent by another instance: ${track.name}`);
          queuedTrackIds.push(track.id);
          continue;
        }
      } catch(e) {
        console.error(`❌ Mark failed: ${track.name} –`, e.message);
        continue;
      }
...
(async () => {
  ...
  // On startup: clear all sentToSpotify flags in Firebase.
  // After a restart Spotify's queue is empty anyway, so all songs must be re-sent.
  // Without this, songs flagged sentToSpotify:true would be skipped forever.
  try {
    const qRef  = db.collection('state').doc('queue');
    const qSnap = await qRef.get();
    if (qSnap.exists) {
      const tracks  = qSnap.data().tracks || [];
      const cleared = tracks.map(t => ({ ...t, sentToSpotify: false }));
      await qRef.set({ tracks: cleared, updated: Date.now() });
      console.log(`🔄 Starting fresh – cleared sentToSpotify flags on ${tracks.length} songs`);
    } else {
      console.log('🔄 Starting fresh – queue is empty');
    }
  } catch(e) { console.error('❌ Could not clear sentToSpotify flags:', e.message); }
