/**
 * Wunschmusik Background Server v4
 * ──────────────────────────────────
 * FIXES in v3:
...
async function sendPendingSongs() {
  ...
  try {
    ...
    const allTracks = qSnap.data().tracks || [];
    const toSend = allTracks.filter(t => !queuedTrackIds.includes(t.id));
...
app.get('/', (req, res) => res.json({
  status: 'running', service: 'Wunschmusik Server v4',
...
// ── SEARCH PROXY ──────────────────────────────────────────────────────────────
// Members search via this endpoint instead of calling Spotify directly from
// the browser (fixes "Failed to fetch" – browser->Spotify calls are blocked,
// server->Spotify calls work fine).
app.get('/search', async (req, res) => {
  res.set('Access-Control-Allow-Origin', '*');
  const q = (req.query.q || '').toString().trim();
  if (!q) return res.status(400).json({ error: { message: 'Suchbegriff fehlt' } });
  if (!accessToken || tokenExpires < Date.now()) {
    return res.status(503).json({ error: { message: 'Kein g\u00fcltiger Token \u2013 Admin verbinden' } });
  }
  try {
    const r = await spFetch(`https://api.spotify.com/v1/search?q=${encodeURIComponent(q)}&type=track&limit=10`);
    if (!r) return res.status(503).json({ error: { message: 'Kein Token' } });
    if (!r.ok) {
