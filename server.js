...
async function sendPendingSongs() {
  ...
  try {
    ...
    // Bereinigung: IDs aus queuedTrackIds entfernen die nicht mehr in Firebase-Queue sind
    // Das verhindert dass Spotify-Queue-Verlust (Session-Ende, Neustart) Songs blockiert
    const firebaseIds = allTracks.map(t => t.id);
    queuedTrackIds = queuedTrackIds.filter(id => firebaseIds.includes(id));
