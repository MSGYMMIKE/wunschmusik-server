...
(async () => {
  await refreshToken();
  await loadDevice();
  // Do NOT pre-load queuedTrackIds - always send all queued songs fresh to Spotify on startup
  // This ensures songs are never stuck in Firebase queue without being sent
  console.log('📋 Starting fresh – will send all queued songs to Spotify');
