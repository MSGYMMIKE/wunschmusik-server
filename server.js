...
async function sendPendingSongs() {
  ...
  try {
    ...
    // AUTO-RESET: Wenn alle Songs als "gesendet" markiert sind aber noch in Firebase Queue
    // → bedeutet Spotify hat sie verloren (nach Ausfall/Neustart)
    // → queuedTrackIds zurücksetzen damit sie erneut gesendet werden
    if (allTracks.length > 0 && allTracks.every(t => queuedTrackIds.includes(t.id))) {
      console.log('⚠️  Alle Songs als gesendet markiert aber noch in Queue – Reset queuedTrackIds');
      queuedTrackIds = [];
    }
