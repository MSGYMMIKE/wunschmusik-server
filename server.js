...
// ── SPOTIFY KEEP-ALIVE ────────────────────────────────────────────────────────
// Alle 2 Minuten Gerät neu laden – verhindert Verbindungsverlust bei langer Pause
setInterval(async () => {
  if (!accessToken) return;
  try {
    await loadDevice();
    if (deviceId) console.log(`🔌 Device check OK: ${deviceId}`);
    else console.log('⚠️  Kein Spotify-Gerät gefunden – bitte Spotify am Studio-PC starten');
  } catch(e) {}
}, 2 * 60 * 1000);
